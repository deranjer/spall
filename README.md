...
7. **No silent correctness tradeoffs under load.** Queue/rate-limit expensive actions and expose overload metrics. Do not delete gameplay debris, classify unknown support as permanent support, or drop topology events to hit a frame-rate target. A game may explicitly opt specific expendable fragments into the [optional debris lifetime](docs/debris-lifetime.md); this is declared material destruction, defaults off, and is not enabled in existing acceptance workloads.
