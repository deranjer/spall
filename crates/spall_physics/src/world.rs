...
/// Builds the Rapier mass properties for `props`, diagonalising the full inertia
/// tensor about the centre of mass into principal axes + moments.
fn rapier_mass_properties(props: BodyMassProperties) -> MassProperties {
...
impl PhysicsWorld {
    ...
    pub fn add_body(&mut self, spec: BodySpec) -> BodyId {
        ...
        if let Some(props) = spec.mass_properties {
            ...
            rb.set_additional_mass_properties(rapier_mass_properties(props), false);
    ...
    pub fn set_mass_properties(&mut self, id: BodyId, props: BodyMassProperties) {
        ...
        let body = entry.body;
        let rb = &mut self.bodies[body];
        rb.set_additional_mass_properties(rapier_mass_properties(props), true);
        rb.recompute_mass_properties_from_colliders(&self.colliders);
    }
    ...
    /// Rapier derived from the collider at its uniform density.
