...
fn a_mixed_density_split_installs_fine_grid_mass_properties_into_the_body() {
    ...
    for (label, cx) in [("adapter", com[0]), ("live body", live_com[0])] {
        assert!(
            (f64::from(cx) - analytic.com_m[0]).abs() < 2e-3,
            "{label} COM_x {cx} vs analytic {}",
            analytic.com_m[0]
        );
        assert!(
            f64::from(cx) < geometric_centre_x - 0.05,
            "{label} COM_x {cx} tracks the density shift, not the centroid {geometric_centre_x}"
        );
    }
