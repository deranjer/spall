...
(`docs/reviews/2026-09-10-g3-residency-handling.md`). **Slices A–E implemented.**
Residency is a **default-off** knob on both ends (`ServeConfig.residency`,
`ClientNetConfig.client_residency`). Row 7 (server eviction) and row 8b
(traverse-away-and-back with client + server eviction, save, restart) both have
passing multi-process evidence. This document is the frozen contract; it does
not certify a gate. Remaining refinements (`ResidencyController` unification,
durable-store backing, incremental capture, logical-terrain predicted collider)
are tracked as follow-ups, not slices.
...
## Post-merge implementation qualification

The [post-merge review](../reviews/2026-09-10-eng-30-post-merge.md) distinguishes
delivered logical-eviction correctness from this contract's remaining durable
acknowledgment, dependency pinning, bounded capture and memory requirements.
Slices D/E use in-memory backing and full reload before checkpoints; their
completion labels below describe those increments, not fulfillment of every
contract requirement. The row 8b fixture has landed (E2); references below to
it as proposed are historical. Increment 12 validates reload candidates before
...
| **E2 — client residency in the session + row 8b traversal** ✅ *(qualified by increment 12)* | `spall_client::ClientResidencyPass` — a mover-loop pass with a global 4-request/step reload ceiling, completed-load and budget-miss accounting. Its derived collision dirty key follows resident cache content while the authoritative hash remains logical and placement-independent. `ClientNetConfig.client_residency` remains default-off. `ClientSummary` v2 exposes evictions, requested/completed reloads, budget misses, and transaction gaps caused by evicted geometry. | `fixtures/scenarios/t23-g3-traversal.json` now has enforced `residency_assertions`: nonzero server/client eviction and completed reloads, an edit gap on evicted geometry, `>=20 m` outbound and `<=12 m` final return displacement. Measured increment-12 clean run: `23.38 m` out → `10.03 m` final, server `5/3` evict/reload, client `6/4/4` evict/request/complete, one evicted-transaction gap, grounded ratio `0.993`; server + clients + replay + restart + reconnect converge to `2a47eda7…`. Disabling either residency pass fails the configured counters. Prediction-safe traversal under `2%` loss, durable-store backing, exact-revision acknowledgements, dependency pins, and bounded checkpoint capture remain open. |
