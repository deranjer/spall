...
- **Increment 5 (residency-aware world hash — design).** Row 7 (live resident-cache eviction in `serve()`) is blocked: `SimWorld::world_hash()` / `ReplicaWorld::world_hash()` fold **resident bricks only** and both ends converge by comparing that value, so evicting a clean terrain brick mid-run breaks convergence and replay. T18 built `ResidencyController` (+ a stable checkpoint hash) but deliberately left `serve()` unwired for this reason. `docs/reports/G3-residency-hash.md` is the design: retain each evicted brick's `(revision, content_hash)` digest (the field `GraphBrickMeta` lacks), add `residency_world_hash(world, digests)` that is bit-identical to `world_hash()` when nothing is evicted and is computed identically on both ends from locally-retained digests (no wire traffic), and have `serve()` report it only when residency is enabled (default off). Design only — no code; awaiting integrator sign-off before the serve-loop wiring increment.
- **Increment 6+ (later).** The residency wiring itself, traverse-away/back, the join-duration budget, and the G4 eight-client workload + soak.
