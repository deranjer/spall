...
- **Increment 14+ (later).** Durable exact-revision backing acknowledgement and incremental checkpoint capture; the join-duration budget (row 11); live catch-up exhaustion (row 10); the G4 eight-client workload + soak (rows 12–14); full-envelope player separation + region-to-region traversal (row 2).
