...
The current [G3 report](reports/G3.md) records increments through 33: the
passing row 11 impaired join-budget run, then row 7's pin-lifetime and
capacity-admission enforcement pass. Row 7's durable exact-revision
acknowledgement, incremental checkpoint capture, and the dormant
`ResidencyController`/`ClientResidency` policy-engine merge remain open.
Earlier increment notes below are historical evidence, not the current
