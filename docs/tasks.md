...
The current [G3 report](reports/G3.md) records increments through 33 (row 7's
pin-lifetime and capacity-admission enforcement pass), plus a dedicated
increment 16 audit that confirmed the evict-time ack-before-evict gate was
already sound and found and fixed a real, narrower gap in the checkpoint
capture path's unverified backing read. Row 7's incremental checkpoint
capture, a lower-severity unverified-read instance in the baseline/repair
paths, and the dormant `ResidencyController`/`ClientResidency` policy-engine
merge remain open. Earlier increment notes below are historical evidence, not
the current remaining-work queue.
...
- **Increment 16 (row 7 — durable exact-revision backing acknowledgement, audit + fix).** Coordinator review (`docs/reports/ENG-30-row7-remaining.md`, 2026-09-18) explicitly cautioned against reopening the increment-27 ack-before-evict fix "merely because it uses a different API shape than the old controller" and asked for a demonstrated-need audit first. The audit, by direct code inspection: the evict-time gate (`ResidencyPass::run` capturing then evicting, same tick, same `&mut SimWorld` borrow, no concurrent-mutation window possible) is already race-free and revision-exact for both `MemoryBacking` and `DiskBrickBacking` — no change needed there. `spall_store::ResidencyStore`'s deliberate `synchronous=NORMAL` pragma (vs. the checkpoint/journal `Writer`'s hard-required `synchronous=FULL`) is never load-bearing for crash recovery either, since `spall_store::recover` reconstructs the durable world exclusively from `world.db`, independent of `residency.db` — traced through `recover_conn_from`, not assumed. The audit *did* find a real, different gap: `ResidencyPass::capture_checkpoint` (the bounded-capture path added post-merge) read evicted-brick records from the backing straight into the **durable** (`synchronous=FULL`) checkpoint with no verification against the retained digest — unlike `SimWorld::reload_brick`, which validates a backing candidate with `EvictedBricks::verify_candidate` before ever publishing it into the live world. A stale/wrong backing record could therefore silently enter a durably-committed save file while `checkpoint.world_hash` (computed from the digest, not from the pushed bytes) kept reporting the correct value. Fixed with one verification step reusing the existing `BrickDigest`/`EvictedBricks::verify_candidate` machinery — no API reshape, no new trait. Falsified with a fault-injection test (`capture_checkpoint_fails_closed_on_a_backing_record_that_disagrees_with_the_retained_digest`) that forges a wrong backing record and confirms `capture_checkpoint` now returns the new `PersistError::EvictedBrickDigestMismatch` instead of publishing it; confirmed to fail (silently accept the forgery) against the pre-fix code by temporarily disabling the check and re-running. Evidence: `cargo test -p spall_server --test residency_pass` (12/12, up from 11); full `cargo fmt --all --check` / `cargo clippy --workspace --all-targets --all-features -- -D warnings` / `cargo test --workspace --all-features` green (`spall_net::separate_process_transport` passed, not flaky this run); `cargo xtask scenario --name t23-g3-residency` / `t23-g3-residency-disk` / `t23-g3-traversal` all converge to their established agreed hashes (`cac2893c…d18e6de`, `cac2893c…d18e6de`, `2a47eda7…3b51d8`) unchanged; `cargo xtask crash-test --suite persistence` (12/12) and `cargo test -p spall_store --test abrupt_crash` unaffected; `cargo xtask check` clean. **Also found, deliberately not fixed in this bounded increment**: `baseline.rs`'s `baseline_volume` / `snapshot_world` / `logical_brick_repair_patch` read the backing the same unverified way for late-join baselines and repair patches sent to clients — lower severity (a network resync/repair cycle, not durable corruption), flagged for a future increment rather than folded in here.
- **Increment 17+ (later).** Incremental (non-full-reload) checkpoint capture; the baseline/repair-path unverified-read instances flagged above; the join-duration budget (row 11); live catch-up exhaustion (row 10); the G4 eight-client workload + soak (rows 12–14); full-envelope player separation + region-to-region traversal (row 2).
...
- **Increment 15+ (later).** Durable exact-revision backing acknowledgement and incremental checkpoint capture; the join-duration budget (row 11); live catch-up exhaustion (row 10); the G4 eight-client workload + soak (rows 12–14); full-envelope player separation + region-to-region traversal (row 2).
