...
The current [G3 report](reports/G3.md) records increments through 34: the
passing row 11 impaired join-budget run, row 7's pin-lifetime and
capacity-admission enforcement pass, then row 7's incremental (non-full-reload)
checkpoint capture. Row 7's durable exact-revision acknowledgement and the
dormant `ResidencyController`/`ClientResidency` policy-engine merge remain
open. Earlier increment notes below are historical evidence, not the current
...
- **Increment 16+ (later).** Durable exact-revision backing acknowledgement; the join-duration budget (row 11); live catch-up exhaustion (row 10); the G4 eight-client workload + soak (rows 12–14); full-envelope player separation + region-to-region traversal (row 2).
