...
struct CaptureArgs {
    ...
    /// `g1-networked-destruction` cut sequence with GPU pass timings), or
    /// `g2-frames` (T15 — per-pass GPU frame-cost percentiles over the still
    /// lighting fixtures at a fixed 1920x1080).
    #[arg(long)]
    scene: Option<String>,
